<%@ page language="java" import="java.util.*,java.util.*,com.xiuson.valuebean.*" pageEncoding="UTF8"%>
...
<html>
  ...
  <%  }
	...
	int ipSum = Integer.parseInt(session.getAttribute("ipSum").toString());	
   %>
   <table border="1"> 
   <tr>
   <td>id</td><td>用户代理</td><td>上一访问页面</td><td>请求的方法</td><td>协议</td><td>请求的URL</td><td>当前文件绝对路径</td><td>客户端地址</td><td>客户端名</td><td>服务器名</td>
   <td>客户端所请求的脚本文件的文件路径</td><td>服务器端口</td><td>请求的时间</td><td>客户端字符编码</td><td>查询的字符串</td><td>路径信息</td><td>远程用户</td><td>客户端接收的语言</td><td>客户端接收的编码</td><td>上一次访问时间</td>
   </tr>
   <%
  		int i = 0;
  		IPBean ipBean;
		...
		<td><%SimpleDateFormat format  = new SimpleDateFormat("yyyy年MM月dd日 HH:mm:ss"); 
		try{
		Date date = new Date(Long.parseLong(ipBean.getLastAccessed()));
