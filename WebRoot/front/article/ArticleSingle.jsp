<%@ page language="java" import="java.util.*,com.xiuson.valuebean.*" pageEncoding="UTF8"%>
<%
...
<html>
  ...
  <!-- 包含头文件 -->
  <tr> <td colspan="20" ><jsp:include page="/front/view/FrontTop.jsp" /></td></tr>
  <tr>
  		<!-- 包含侧栏文件 -->
        ...
        <tr>
        <td align="center">
        <b><font style="font-size:25px"><%=single.getTitle() %></font></b>
  		</td>
  		</tr>
  		<tr><td><%=single.getCreate() %>:<%=single.getInfo() %></td></tr>
  		<tr><td valign="top"><%=single.getContent() %></tr></td>
  		</table>
  	...
  	<!--	 以下是显示文章评论代码：-->
  		...
  		out.println("<tr><td align='center'>【文章评论共"+reviewSum+"条】</td></tr>");
  		int i = 0;
  		while (i < reviewList.size()) {
  			ReviewBean reviewSingle = (ReviewBean)reviewList.get(i);
  		 %>
  		 <tr><td style="text-indent:20">【】<b><%=reviewSingle.getAuthor() %><%System.out.println("test***********author"+reviewSingle.getAuthor()); %></b></td></tr>
  		 <tr align="right" ><td><%=reviewSingle.getSdTime() %> &nbsp;&nbsp;</td></tr>
  		 <tr><td colspan="2" valign="top" > <%=reviewSingle.getContent() %></tr></td>
  		...
  		out.println("</table>");
  		 ...
  		 %>
  		 <a href="ArticleServlet?action=read&reviewBegin=<%=i+1%>&reviewCount=10&id=<%=single.getId() %>"><%=i+1 %></a>
  		 <%if(i!=0 && i%10 == 0) 
  		      out.println(" <br/>");%>		
  		 <%} %>	 		 
  		<!--  以下是填写评论内容的代码： -->	 
  		...
  		<!-- 包含页尾文件 -->
  		<tr><td colspan="2"><jsp:include page="../view/FrontEnd.jsp"/></td> </tr>
  		</table>	
  		<%
  		}
  		 %>
