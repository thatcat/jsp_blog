...
public class IPDao {
	...
	public List queryIP(int begin,int count) throws SQLException {
		...
		if (begin == 0 && count == 0) {
			...
			sql = "select * from ip order by id desc limit "+(begin-1)*10+","+count+"";
	...
	public void addIP(IPBean ipBean) {
		...
		System.out.println("length:"+referer+"***"+"***"+method+"***"+protocol+"***"+requestUrl+"***"+realPath+
				"***"+remoteAddr+"***"+remoteHost+"***"+serverName+"***"+serverPath+"***"+serverPort+"***"+time
				+"***"+characterEncoding+"***"+queryString+"***"+pathInfo+"***"+remoteUser+"***"+acceptLanguage+"***"+acceptEncoding+"***"+userAgent+"***"+lastAccessed);
		
		System.out.println("length:"+referer.length()+"***"+"***"+method.length()+"***"+protocol.length()+"***"+requestUrl.length()+"***"+realPath.length()+
				"***"+remoteAddr.length()+"***"+remoteHost.length()+"***"+serverName.length()+"***"+serverPath.length()+"***"+serverPort.length()+"***"+time.length()
				+"***"+characterEncoding.length()+"***"+queryString.length()+"***"+pathInfo.length()+"***"+remoteUser.length()+"***"+acceptLanguage.length()+"***"+acceptEncoding.length()+"***"+userAgent.length()+"***"+lastAccessed.length());
		...
		String sql = "insert into ip(referer ,method ,protocol,requestUrl,realPath,remoteAddr,remoteHost," +
				...
				" values('"+referer +"','"+method +"','"+protocol +"','"+requestUrl +"','"+realPath +"'" +
						",'"+remoteAddr +"','"+remoteHost +"','"+serverName +"','"+serverPath +"','"+serverPort +"','"+time +
						"','"+characterEncoding+"','"+queryString +"','"+pathInfo +"','"+remoteUser +"','"+acceptLanguage +"','"+acceptEncoding +"','"+userAgent +"','"+lastAccessed+"')";
